...
export async function GET(req: NextRequest) {
  const leaveNo = req.nextUrl.searchParams.get('leave_no');
  if (!leaveNo) {
    return NextResponse.json({ error: 'Missing required parameter: leave_no' }, { status: 400 });
  }
  try {
    ...
    const leave = leaves.find((l: any) => l.No === leaveNo);
    if (!leave) {
      return NextResponse.json({ error: 'Leave application not found' }, { status: 404 });
    }
    return NextResponse.json(leave);
  } catch (error: any) {
    return NextResponse.json({ error: 'Failed to fetch leave application details', details: String(error) }, { status: 500 });
  }
}
