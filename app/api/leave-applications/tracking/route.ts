...
export async function GET(req: NextRequest) {
  try {
    console.log('\n╔════════════════════════════════════════════════════╗');
    console.log('║  GET LEAVE APPLICATION TRACKING (All Requests)    ║');
    ...
    console.log(`\n📊 Total Leave Application Records from BC: ${requests.length}`);
    console.log('╚════════════════════════════════════════════════════╝\n');
    
    return NextResponse.json(requests);
  } catch (error: any) {
    console.error('Error in leave tracking:', error.message);
    return NextResponse.json([], { status: 200 });
  }
}
