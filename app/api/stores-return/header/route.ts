...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('\n╔════════════════════════════════════════════════════════════════╗');
    console.log('║  CREATE STORES RETURN HEADER REQUEST                            ║');
    console.log('╚════════════════════════════════════════════════════════════════╝\n');
    ...
    if (!body.man_number || !body.reason) {
      console.error('❌ Missing required fields in stores return header request');
    ...
    const soapBody = `
      <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:web="urn:microsoft-dynamics-schemas/codeunit/InsertDocuments">
        <soapenv:Header/>
        <soapenv:Body>
          <web:InsertStoresReturnHeader>
            <web:manNumber>${escapeXml(body.man_number)}</web:manNumber>
            <web:storesType>${escapeXml(body.stores_type || 'Return')}</web:storesType>
    ...
    console.log(`\n╔════════════════════════════════════════════════════════════════╗`);
    console.log(`║  SOAP REQUEST - InsertStoresReturnHeader                       ║`);
    console.log(`╚════════════════════════════════════════════════════════════════╝\n`);
    ...
    console.log(`   • SOAPAction: urn:microsoft-dynamics-schemas/codeunit/InsertDocuments:InsertStoresReturnHeader`);
    ...
    const response = await fetch(SOAP_URL, {
      ...
      headers: {
        ...
        'SOAPAction': 'urn:microsoft-dynamics-schemas/codeunit/InsertDocuments:InsertStoresReturnHeader',
    ...
    console.log(`╔════════════════════════════════════════════════════════════════╗`);
    console.log(`║  SOAP RESPONSE - InsertStoresReturnHeader                      ║`);
    console.log(`╚════════════════════════════════════════════════════════════════╝\n`);
    ...
    if (response.status !== 200) {
      console.error(`❌ Error: Stores return header creation failed`);
      ...
      return NextResponse.json(
        {
          success: false,
          message: 'Failed to create stores return header',
          error: errorMessage,
        },
    ...
    console.log(`✅ SUCCESS: Stores return header created`);
    console.log(`📊 Document Number: ${documentNumber}\n`);

    return NextResponse.json({
      success: true,
      document_number: documentNumber,
      message: 'Stores return header created successfully',
    });
  } catch (error: any) {
    console.error('\n❌ Unexpected error in stores return header creation:');
    console.error(error);
    console.log(`\n════════════════════════════════════════════════════════════════\n`);
