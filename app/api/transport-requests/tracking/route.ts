...
export async function GET(req: NextRequest) {
  try {
    console.log('\n╔════════════════════════════════════════════════════╗');
    console.log('║  GET TRANSPORT TRACKING (All Requests)             ║');
    ...
    console.log(`\n📊 Total Transport Tracking Records from BC: ${requests.length}`);
    console.log('╚════════════════════════════════════════════════════╝\n');
    
    return NextResponse.json(requests);
  } catch (error: any) {
    console.error('Error in transport tracking:', error.message);
    return NextResponse.json([], { status: 200 });
  }
}
