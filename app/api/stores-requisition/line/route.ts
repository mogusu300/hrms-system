...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('\n╔════════════════════════════════════════════════════════════════╗');
    console.log('║  INSERT STORES REQUISITION LINE REQUEST                        ║');
    console.log('╚════════════════════════════════════════════════════════════════╝\n');
    ...
    const soapBody = `
      <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:web="urn:microsoft-dynamics-schemas/codeunit/InsertDocuments">
        <soapenv:Header/>
        <soapenv:Body>
          <web:InsertStoresRequisitionLine>
            ...
            <web:accountType>${body.account_type || 0}</web:accountType>
    ...
    console.log(`\n╔════════════════════════════════════════════════════════════════╗`);
    console.log(`║   SOAP REQUEST - InsertStoresRequisitionLine                   ║`);
    console.log(`╚════════════════════════════════════════════════════════════════╝\n`);
    ...
    console.log(`   • SOAPAction: urn:microsoft-dynamics-schemas/codeunit/InsertDocuments:InsertStoresRequisitionLine`);
    ...
    const response = await fetch(SOAP_URL, {
      ...
      headers: {
        ...
        'SOAPAction': 'urn:microsoft-dynamics-schemas/codeunit/InsertDocuments:InsertStoresRequisitionLine',
    ...
    console.log(`╔════════════════════════════════════════════════════════════════╗`);
    console.log(`║   SOAP RESPONSE - InsertStoresRequisitionLine                  ║`);
    console.log(`╚════════════════════════════════════════════════════════════════╝\n`);
    ...
    if (response.status !== 200) {
      console.error(`❌ Error: Stores requisition line submission failed`);
      ...
      return NextResponse.json(
        {
          success: false,
          message: 'Failed to submit stores requisition line',
          error: errorMessage,
        },
    ...
    console.log(`✅ SUCCESS: Stores requisition line submitted`);
    console.log(`📋 Result: ${result}\n`);

    return NextResponse.json({
      success: true,
      result: result,
      message: 'Stores requisition line submitted successfully',
    });
  } catch (error: any) {
    console.error('\n❌ Unexpected error in stores requisition line submission:');
    console.error(error);
    console.log(`\n════════════════════════════════════════════════════════════════\n`);
