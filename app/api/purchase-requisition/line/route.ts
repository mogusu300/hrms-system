...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('\n╔════════════════════════════════════════════════════════════════╗');
    console.log('║  INSERT PURCHASE REQUISITION LINE REQUEST                      ║');
    console.log('╚════════════════════════════════════════════════════════════════╝\n');
      ...
      <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:web="urn:microsoft-dynamics-schemas/codeunit/InsertDocuments">
        ...
        <soapenv:Body>
          <web:InsertRequisitionLine>
            ...
            <web:accountType>${body.account_type || 2}</web:accountType>
    ...
    console.log(`\n╔════════════════════════════════════════════════════════════════╗`);
    console.log(`║   SOAP REQUEST - InsertRequisitionLine                        ║`);
    console.log(`╚════════════════════════════════════════════════════════════════╝\n`);
    ...
    console.log(`   • SOAPAction: urn:microsoft-dynamics-schemas/codeunit/InsertDocuments:InsertRequisitionLine`);
    ...
    const response = await fetch(SOAP_URL, {
      ...
      headers: {
        ...
        'SOAPAction': 'urn:microsoft-dynamics-schemas/codeunit/InsertDocuments:InsertRequisitionLine',
    ...
    console.log(`╔════════════════════════════════════════════════════════════════╗`);
    console.log(`║   SOAP RESPONSE - InsertRequisitionLine                       ║`);
    console.log(`╚════════════════════════════════════════════════════════════════╝\n`);
    ...
    if (response.status !== 200) {
      console.error(`❌ Error: Purchase requisition line submission failed`);
      ...
      return NextResponse.json(
        {
          success: false,
          message: 'Failed to submit purchase requisition line',
          error: errorMessage,
        },
    ...
    console.log(`✅ SUCCESS: Purchase requisition line submitted`);
    console.log(`📋 Result: ${result}\n`);

    return NextResponse.json({
      success: true,
      result: result,
      message: 'Purchase requisition line submitted successfully',
    });
  } catch (error: any) {
    console.error('\n❌ Unexpected error in purchase requisition line submission:');
    console.error(error);
    console.log(`\n════════════════════════════════════════════════════════════════\n`);
