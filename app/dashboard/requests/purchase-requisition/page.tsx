...
export default function PurchaseRequisitionPage() {
        ...
        if (sessionResponse.ok) {
          ...
          if (empNumber) {
            ...
            await fetch(`/api/employee-details?manNumber=${encodedEmpNumber}`).catch(err => {
              console.log('Employee details fetch skipped, not required for purchase requisition');
            });
          }
        }
      } catch (error) {
  ...
  const handleSubmitHeader = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload = {
        ...
        purchase_type: 0, // Fixed value
        currency_code: formData.currencyCode,
        description: formData.description,
        order_type: 1, // Fixed value
      };

      const response = await fetch('/api/purchase-requisition/header', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to create purchase requisition header');
      }
      ...
      toast({
        title: 'Success',
        description: `Purchase requisition created! Document #${documentNumber}`,
      });
    } catch (error: any) {
  ...
  const submitLine = async (item: SelectedItem) => {
    ...
    try {
      ...
      const response = await fetch('/api/purchase-requisition/line', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto space-y-8">
          {/* Header */}
          <div className="pt-4">
            <h1 className="text-4xl font-bold text-gray-900">Purchase Requisition</h1>
            <p className="text-gray-600 mt-2 text-lg">Create a new purchase requisition request</p>
          </div>
