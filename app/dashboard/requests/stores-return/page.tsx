...
export default function StoresReturnPage() {
        ...
        if (sessionResponse.ok) {
          ...
          if (empNumber) {
            ...
            await fetch(`/api/employee-details?manNumber=${encodedEmpNumber}`).catch(err => {
              console.log('Employee details fetch skipped, not required for stores return');
            });
          }
        }
      } catch (error) {
  ...
  const handleSubmitHeader = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload = {
        ...
        stores_type: 'Return', // Fixed value for stores return
        reason: formData.reason,
      };

      const response = await fetch('/api/stores-return/header', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to create stores return header');
      }
      ...
      toast({
        title: 'Success',
        description: `Stores return created! Document #${documentNumber}`,
      });
    } catch (error: any) {
      console.error('Error submitting header:', error);
      toast({
        title: 'Error',
        description: error.message || 'Failed to create stores return',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const addItem = (item: AvailableItem) => {
    ...
    toast({
      title: 'Item Added',
      description: `${item.description} has been added to the return`,
    });
  };
  ...
  const submitLine = async (item: SelectedItem) => {
    ...
    try {
      const payload = {
        ...
        account_type: 1, // Stores return account type (different from requisition)
      ...
      const response = await fetch('/api/stores-return/line', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto space-y-8">
          {/* Header */}
          <div className="pt-4">
            <h1 className="text-4xl font-bold text-gray-900">Stores Return</h1>
            <p className="text-gray-600 mt-2 text-lg">Create a new stores return request</p>
          </div>
          ...
          <Card className="p-8 border-2 border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-lg font-semibold text-gray-900">Step 1: Create Return Header</h2>
            </div>
            ...
            <form onSubmit={handleSubmitHeader} className="space-y-6">
              ...
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-100">
                <p className="text-sm font-semibold text-gray-700">
                  Type: <span className="text-blue-600 font-bold">Return</span>
                </p>
              </div>
        ...
        <Card className="p-6 border-2 border-green-200 rounded-2xl bg-green-50">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="text-green-900">Return</p>
            </div>
          </div>
        </Card>
