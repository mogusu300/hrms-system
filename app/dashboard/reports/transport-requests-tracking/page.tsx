...
interface TransportRequestRecord {
  Document_No: string;
  Pay_to_Code: string;
  Pay_to_Name: string;
  Reason: string;
...
export default function TransportRequestsTrackingPage() {
  const [detailsLoading, setDetailsLoading] = useState(false);
  const [detailsError, setDetailsError] = useState<string | null>(null);
  const [search, setSearch] = useState("");
  const [records, setRecords] = useState<TransportRequestRecord[]>([]);
  const [loading, setLoading] = useState(false);
  const [detailsOpen, setDetailsOpen] = useState(false);
  const [selectedRequest, setSelectedRequest] = useState<TransportRequestRecord | null>(null);
  const [fetchError, setFetchError] = useState<string | null>(null);

  useEffect(() => {
    setLoading(true);
    setFetchError(null);
    fetch("/api/transport-requests/tracking")
      .then(res => {
  ...
  const handleViewDetails = async (request: TransportRequestRecord) => {
    ...
    <div className="p-6 space-y-6">
      <Card className="shadow-md">
        <CardHeader>
          ...
          <CardDescription>Track transport requests</CardDescription>
        </CardHeader>
        <CardContent>
