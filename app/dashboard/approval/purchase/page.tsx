...
export default function PurchaseApprovalPage() {
  ...
  const [records, setRecords] = useState<PurchaseApprovalRecord[]>([]);
  const [loading, setLoading] = useState(false);
  const [detailsOpen, setDetailsOpen] = useState(false);
  const [selectedPurchase, setSelectedPurchase] = useState<PurchaseApprovalRecord | null>(null);
  ...
  const handleViewDetails = async (purchase: PurchaseApprovalRecord) => {
  ...
  const approveRequest = async (purchase: PurchaseApprovalRecord) => {
    setApproving(true);
    try {
      const res = await fetch('/api/purchase-tracking/approve', {
        method: 'POST',
      ...
      toast({ title: "Success", description: "Purchase approved successfully", variant: "default" });
    ...
    <div className="p-6 space-y-6">
      <Card className="shadow-md">
        <CardHeader>
          ...
          <CardDescription>Approve purchase requisitions</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <TableBody>
                  {filteredRecords.map((record, idx) => (
                    ...
                    >
                      ...
                      <TableCell>{record.Status_RP}</TableCell>
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="py-4 space-y-2">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900 font-medium">{selectedPurchase.Amount?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
                <div>
          ...
          <div className="flex gap-2 mt-6">
            <Button 
              onClick={() => approveRequest(selectedPurchase!)}
              disabled={approving}
