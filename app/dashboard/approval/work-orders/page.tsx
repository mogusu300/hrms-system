...
interface WorkOrderApprovalRecord {
  No: string;
...
export default function WorkOrdersApprovalPage() {
  ...
  const [records, setRecords] = useState<WorkOrderApprovalRecord[]>([]);
  const [loading, setLoading] = useState(false);
  const [detailsOpen, setDetailsOpen] = useState(false);
  const [selectedWorkOrder, setSelectedWorkOrder] = useState<WorkOrderApprovalRecord | null>(null);
  ...
  const handleViewDetails = async (workOrder: WorkOrderApprovalRecord) => {
  ...
  const approveRequest = async (workOrder: WorkOrderApprovalRecord) => {
    setApproving(true);
    try {
      const res = await fetch('/api/work-orders/approve', {
        method: 'POST',
      ...
      toast({ title: "Success", description: "Work order approved successfully", variant: "default" });
      ...
      <Card className="shadow-md">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle>Work Orders Approval</CardTitle>
          </div>
          <CardDescription>Approve work orders</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex justify-end gap-2 pt-4 border-t mt-6">
            <Button onClick={() => approveRequest(selectedWorkOrder!)} disabled={approving} style={{ backgroundColor: '#10b981', color: 'white' }}>
              {approving ? "Approving..." : "Approve Work Order"}
            </Button>
            <DialogClose asChild>
