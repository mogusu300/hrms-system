...
interface StoresApprovalRecord {
  Document_No: string
...
interface StoreRequestDetails extends StoresApprovalRecord {
  Amount?: number
  Reason?: string
  Approver_ID?: string
  Sent_By?: string
...
export default function StoresApprovalPage() {
  const [records, setRecords] = useState<StoresApprovalRecord[]>([]);
  ...
  useEffect(() => {
    async function fetchRecords() {
      setLoading(true);
      try {
        const res = await fetch("/api/stores-tracking");
  ...
  async function approveRequest(document_no: string) {
    setApproveLoading(true);
    try {
      const res = await fetch("/api/stores-tracking/approve", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ document_no })
      });
      if (!res.ok) throw new Error('Failed to approve');
      toast({ title: `Store request ${document_no} approved successfully` });
      setDetailsOpen(false);
      // Refresh the list
      const updatedRes = await fetch("/api/stores-tracking");
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <Card className="shadow-md">
        <CardHeader>
          <CardTitle>Stores Approval</CardTitle>
          <CardDescription>Approve store requests</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {!detailsLoading && requestDetails ? (
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 mt-4">
                <Button 
                  id="approveButton" 
                  className="btn-approve bg-green-600 hover:bg-green-700"
                  onClick={() => approveRequest(requestDetails.Document_No)}
                  disabled={approveLoading}
                >
                  {approveLoading ? "Approving..." : "Approve Store"}
                </Button>
                <DialogClose asChild>
